	...
	procedure :: GetOmegaFromInitial !   uncommented this function, which is written at the end of the file
    ...
    type, extends(TQuintessence) :: TEarlyQuintessence !   Will rename the class later, but i will have to rename it in other .f90 files so be careful
        real(dl) :: n = 3._dl !   Won't use
        ...
        logical :: init_failed_flag = .false. ! Flag to indicate failure in initialization
    ...
    a2=a**2	!   - Remember that in CAMB notation dots denote derivatives with respect to conformal time
    phi = y(1)
    phidot = y(2)/a2
    ...
    real(dl) function TQuintessence_phidot_start(this,phi) !   - I won't change this but it may be interesting to leave this here.
    class(TQuintessence) :: this
    real(dl) :: phi
    ...
    !   - This function can be used to get phi(a) and phidot(a) by interpolation. When the Init function is called it evolves the background field
    ...
    clxq=y(w_ix) !   - clxq = \delta \phi is the field perturbation
    vq=y(w_ix+1)
    ayprime(w_ix)= vq
    ayprime(w_ix+1) = - 2*adotoa*vq - k*z*phidot - k**2*clxq - a**2*clxq*this%Vofphi(phi,2)
    !   - This is the quintessence perturbation equation in synchronous gauge (check my notes on scalar fields!)
    ...
    this%init_failed_flag = .false. ! Initialize flag to false
...
! YX modified init function start
    astart=1d-9
    atol=1d-8
	if (this%search_for_initialphi .eqv. .true.) then
        !   - This code bit outputs omega_phi in terms of initial_phi and stops the code. This is done in order to calibrate the binary search initial window and to
    ...
    !   - Binary search algorithm. This is an important step of the code. The current energy density of the field (and therefore Omega_de)
    ...
    do i=1, npoints-1
        ...
        ! fde is the dark energy fraction
